...
class CurrentLocationViewController: UIViewController, CLLocationManagerDelegate, CAAnimationDelegate
{
   ...
   func configureGetButton()
   {
	 ...
	 //Add Animation Spinner while waiting on Location to Update / GeoCode
	 if view.viewWithTag(spinnerTag) == nil
	 {
	    let spinner = UIActivityIndicatorView(style: .medium)
	    spinner.center = messageLabel.center
	    spinner.center.y = spinner.center.y + (spinner.bounds.size.height / 2) + 25
   ...
   //Formats the Address Output after GeoCoding
   ...
   //Includes Animation for Exiting the Logo Screen
   func hideLogoView()
   {
      if logoVisible == false
      {
